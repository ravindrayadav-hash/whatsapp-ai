 ...
 */
export async function scrapeGroupMessages(
  page,
  groupName,
  cursor = null,
  limit = MSG_LIMIT,
...
async function returnToSearchState(page) {
  ...
  const alreadyVisible = await searchInput
...
async function findChatRow(page, groupName) {
  ...
  const spanVisible = await titleSpan
  ...
  const rowByText = page
    ...
    .first();
  if (await rowByText.isVisible({ timeout: 3_000 }).catch(() => false))
    return rowByText;

  // Strategy 3: exact text match
  const byExactText = page.getByText(groupName, { exact: true }).first();
  if (await byExactText.isVisible({ timeout: 3_000 }).catch(() => false))
    return byExactText;
...
async function extractFullHistory(page, limit) {
  ...
  console.log(
    "[Scraper] Phase 1 — loading full message history (scroll to top)...",
  );
  ...
  while (stableRounds < MAX_STABLE_ROUNDS) {
    ...
    if (atBeginning) {
      console.log(
        "[Scraper] Phase 1 — reached chat beginning marker, stopping early.",
      );
      break;
    }
  }
  ...
  console.log(
    "[Scraper] Phase 1 done. Starting Phase 2 (harvest scroll-down)...",
  );
  ...
  const harvest = async () => {
    const rawItems = await page.evaluate(extractMessagesFromDOM, {
      sel: SELECTORS,
      limit: 9999,
    });
    let added = 0;
...
async function extractSinceCursor(page, cursor, limit) {
  ...
  while (!foundBoundary && round < MAX_ROUNDS) {
    ...
    if (atTop) {
      console.log(
        `[Scraper] Reached chat top without finding cursor — accepting all collected messages`,
      );
      break;
    }

...
const RETENTION_DAYS = Math.max(
  Number(process.env.AI_LOG_RETENTION_DAYS) || 90,
  1,
);
...
async function runCleanup() {
  ...
  try {
    ...
    console.log(
      `[Cleanup] Deleted ${deleted} ai_log rows older than ${RETENTION_DAYS} days (cutoff: ${cutoff.toISOString()})`,
    );
  } catch (err) {
...
export function startCleanupJob() {
  ...
  console.log(
    `[Cleanup] Weekly ai_logs cleanup scheduled (${CLEANUP_SCHEDULE} UTC, retention: ${RETENTION_DAYS} days)`,
  );
  return task;
}

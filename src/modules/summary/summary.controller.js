...
export async function triggerProcessing(req, res, next) {
  try {
    ...
    if (!group_name) {
      return res
        .status(400)
        .json({ success: false, message: "groupName param is required" });
    }
    ...
    if (result.status === "skipped") {
      return res
        .status(200)
        .json({ success: true, status: "skipped", reason: result.reason });
    }
...
export async function fetchSummaries(req, res, next) {
  try {
    ...
    if (from && isNaN(from.getTime())) {
      return res
        .status(400)
        .json({ success: false, message: "from must be a valid date" });
    }
    if (to && isNaN(to.getTime())) {
      return res
        .status(400)
        .json({ success: false, message: "to must be a valid date" });
    }

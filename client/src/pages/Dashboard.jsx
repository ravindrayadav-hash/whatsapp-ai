  ...
  return (
    <div className="group-card" onClick={onClick}>
      ...
      <p className="group-meta">
        Last summary:{" "}
        {latest ? timeAgo(latest.createdAt) : "Not yet summarised"}
      </p>
      {latest && (
        <>
          ...
          >
            {latest.requirements?.length > 0 && (
              <span className="badge blue">
                {latest.requirements.length} requirements
              </span>
            )}
            ...
            {latest.action_items?.length > 0 && (
              <span className="badge yellow">
                {latest.action_items.length} actions
              </span>
            )}
          </div>
        </>
        ...
        <div className="groups-grid">
          {groups.map((g) => (
            <GroupCard
              ...
              group={g}
              onClick={() =>
                navigate(`/summaries?group=${encodeURIComponent(g)}`)
              }
            />
          ))}
        </div>

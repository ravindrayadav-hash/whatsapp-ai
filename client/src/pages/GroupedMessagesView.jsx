...
function mapGroupsToMessages(aiGroups, allMessages) {
  ...
  function findMsg(text) {
    ...
    for (let i = 0; i < allMessages.length; i++) {
      ...
      if ((allMessages[i].message || "").trim() === t) {
        usedIdx.add(i);
    ...
    if (prefix.length > 8) {
      for (let i = 0; i < allMessages.length; i++) {
        if (usedIdx.has(i)) continue;
        if ((allMessages[i].message || "").startsWith(prefix)) {
          usedIdx.add(i);
          return allMessages[i];
        }
      }
  ...
  const mapped = aiGroups
    .map((g) => ({
      title: g.title,
      messages: g.messages.map(findMsg).filter(Boolean),
    }))
    .filter((g) => g.messages.length > 0);

  const other = allMessages.filter((_, i) => !usedIdx.has(i));
  if (other.length > 0)
    mapped.push({ title: "Other / Uncategorized", messages: other });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ flex: 1, fontSize: 14, fontWeight: 700, lineHeight: 1.3 }}
        >
          {group.title}
        </span>
          ...
          >
            <span
              style={{
                ...
                borderTopColor: ACTION_CONFIG.find((a) => a.key === loading)
                  ?.color,
              ...
              >
                <span
                  style={{
                    ...
                    color: activeCfg?.color ?? "var(--text-muted)",
                  }}
                >
                  {(activeCfg?.label ?? loading)
                    ? `${ACTION_CONFIG.find((a) => a.key === loading)?.label} Result`
                    : "Result"}
                </span>
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {group.messages.length} msg
                  {group.messages.length !== 1 ? "s" : ""} analyzed
                </span>
              </div>
...
export default function GroupedMessagesView() {
  ...
  function getState(idx) {
    return groupStates[idx] ?? initGroupState();
  }
      ...
      setGroupStates((prev) => {
        ...
        return {
          ...
          [topicIdx]: {
            ...
            results: {
              ...cur.results,
              [actionKey]: { data: res.data, error: null },
            },
          },
        };
      });
      ...
      setGroupStates((prev) => {
        ...
        return {
          ...
          [topicIdx]: {
            ...
            results: {
              ...cur.results,
              [actionKey]: { data: null, error: err.message },
            },
          },
        };
      });
  ...
  function expandAll() {
    if (topics) setExpanded(new Set(topics.map((_, i) => i)));
  }
  ...
  return (
    <div>
      ...
      {hasMore && hasMessages && (
        ...
        >
          Only the first {messages.length} messages are loaded. Results may not
      ...
      {selectedGroup &&
        ...
        !hasTopics && (
          <div className="empty-state">
            ...
            <small>
              Click "Group by Topic" to let AI organize them by requirement
            </small>
            <button
              className="btn"
              onClick={handleGroup}

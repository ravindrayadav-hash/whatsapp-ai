...
function getPreview(action, response) {
  ...
  if (action === 'reply') {
    const t = response.context_summary || response.suggested_replies?.[0]?.message || '';
...
function computeKeyIndices(messages, response) {
  ...
  messages.forEach((m, i) => {
    if (m.image_url) { keySet.add(i); return; }
    // Sender name (>2 chars to avoid false matches) appears in the result
    if (m.sender && m.sender.length > 2 && resultText.includes(m.sender.toLowerCase())) {
      keySet.add(i);
    }
  });
...
function ActionBadge({ type }) {
  const cfg = ACTION_TYPES.find(a => a.key === type) || { label: type, color: 'var(--text-muted)' };
  return (
    <span style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        background: isKey
          ? (isSelected ? 'rgba(37,211,102,0.08)' : 'rgba(37,211,102,0.03)')
    ...
    >
      ...
      {msg.image_url && (
        ...
        >
          <img
            src={msg.image_url}
            alt="attachment"
            style={{ width: 38, height: 38, objectFit: 'cover', borderRadius: 5, display: 'block', border: '1px solid var(--border)' }}
          />
        </a>
      )}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{
          fontSize: 12, lineHeight: 1.45, color: 'var(--text-muted)', margin: 0,
...
function DetailDrawer({ entry, onClose }) {
  ...
  const allIndices = useCallback(() => new Set(messages.map((_, i) => i)), [entry?.id]); // eslint-disable-line
  ...
  function selectAll()     { setSelectedIds(new Set(messages.map((_, i) => i))); }
  function selectNone()    { setSelectedIds(new Set()); }
  function selectKeyOnly() { setSelectedIds(new Set(keySet)); }
  ...
  // ── Active result data ─────────────────────────────────────────────────────
  const activeData     = resultView === 'rerun' && rerunResult ? rerunResult.data     : entry.response;
  const activeMessages = resultView === 'rerun' && rerunResult ? rerunResult.messages : messages;
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: 'min(920px, calc(100vw - 48px))', zIndex: 201,
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)', marginRight: 2 }}>
                Messages
              </span>
              <span style={{
                fontSize: 11, color: selCount > 0 ? cfg?.color : 'var(--text-muted)',
                fontWeight: 600,
              }}>
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: selCount === 0 ? 'var(--surface-alt)' : cfg?.color,
                  color: selCount === 0 ? 'var(--text-muted)' : '#fff',
                  fontSize: 12, fontWeight: 700, cursor: selCount === 0 ? 'not-allowed' : 'pointer',
            ...
            <div style={{
                ...
                return (
                  ...
                  >
                    ...
                    {tab.key === 'rerun' && (
                      <span style={{
                        fontSize: 9, padding: '1px 5px', borderRadius: 3,
                        background: `${cfg?.color}22`, color: cfg?.color, fontWeight: 800,
                      }}>
                        NEW
                      </span>
                    )}
              ...
              {resultView === 'rerun' && rerunResult && (
                <span style={{
                  ...
                  paddingRight: 16, display: 'flex', alignItems: 'center',
                }}>
                  {rerunResult.messages.length} msg{rerunResult.messages.length !== 1 ? 's' : ''} used
                </span>
              )}
            </div>
...
export default function AIHistoryView() {
  ...
  const filterKey = [filterGroup, filterAction, filterFrom, filterTo].join('|');
  ...
  return (
    <div>
      ...
      <div className="toolbar" style={{ flexWrap: 'wrap', gap: 8 }}>
        ...
        <select value={filterGroup} onChange={e => setFilter('group', e.target.value)}>
          <option value="">All groups</option>
          {groups.map(g => <option key={g} value={g}>{g}</option>)}
        </select>

        <select value={filterAction} onChange={e => setFilter('action', e.target.value)}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th style={{ width: 40, textAlign: 'right', paddingRight: 8 }}>#</th>
              ...
              <tbody>
                {logs.map((entry, i) => {
                  ...
                  return (
                    ...
                    >
                      <td style={{ color: 'var(--text-muted)', fontSize: 11, textAlign: 'right', paddingRight: 8 }}>
                        {i + 1}
                      </td>
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        {entry.group_name
                          ? <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--accent)' }}>{entry.group_name}</span>
                      ...
                      <td>
                        <p style={{
                          fontSize: 13, lineHeight: 1.4, margin: 0, maxWidth: 360,
                          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                          color: 'var(--text)',
                        }}>
      ...
      {logs.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'center', padding: '16px 0' }}>
          ...
          {!loading && hasMore && (
            <button className="btn btn-ghost" onClick={loadMore} style={{ fontSize: 13 }}>

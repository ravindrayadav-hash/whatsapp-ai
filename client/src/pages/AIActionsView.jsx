...
const ACTION_CONFIG = [
  { key: 'summarize', label: 'Summarize', desc: 'Requirements & topics',  color: 'var(--accent)'   },
  { key: 'explain',   label: 'Explain',   desc: 'Plain English overview', color: 'var(--info)'     },
  { key: 'reply',     label: 'Reply',     desc: 'Suggested responses',    color: 'var(--warning)'  },
...
function getPreview(action, data) {
  ...
  if (action === 'reply') {
    const t = data.context_summary || data.suggested_replies?.[0]?.message || '';
...
function ImageModal({ url, onClose }) {
  return (
    ...
    >
      <div
        style={{ position: 'fixed', top: 14, right: 16, zIndex: 1001, display: 'flex', gap: 6 }}
...
export default function AIActionsView() {
  ...
  const {
    messages, loading: msgsLoading, error: msgsError,
    hasMore, total, sentinelRef,
  ...
  return (
    <div>
      <span className="back-btn" onClick={() => navigate('/')}>← Dashboard</span>

      <div className="page-header">
        <h1>AI Actions</h1>
        <p>Select messages from a group, then run an AI action to get structured analysis</p>
      </div>
      ...
      <div className="toolbar">
        ...
        {(messages.length > 0 || msgsLoading) && (
          <>
            <button className="btn btn-ghost" onClick={toggleAll} disabled={messages.length === 0}>
              {allSelected ? 'Deselect All' : 'Select All'}
            </button>
            <span style={{ fontSize: 13, color: selectedCount > 0 ? 'var(--accent)' : 'var(--text-muted)' }}>
              {selectedCount > 0
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 16, alignItems: 'start' }}>
          ...
          <div>
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              {msgsError && (
                <div className="error-banner" style={{ margin: 16 }}>{msgsError}</div>
              )}
              {!msgsLoading && !msgsError && messages.length === 0 && (
                <div className="empty-state"><p>No messages found</p></div>
              )}
              {messages.length > 0 && (
                        ...
                        return (
                          <tr
                            ...
                            style={{
                              cursor: 'pointer',
                              background: sel
                                ? m.image_url ? 'rgba(168,85,247,0.08)' : 'rgba(37,211,102,0.07)'
                                : undefined,
                              boxShadow: sel && m.image_url
                          ...
                          >
                            ...
                            <td style={{ whiteSpace: 'nowrap', color: 'var(--accent)', fontWeight: 500 }}>
                              {m.sender}
                            </td>
                              ...
                              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                                {m.image_url && (
                                  <img
                                    src={m.image_url}
                                    alt="attachment"
                                    onClick={(e) => { e.stopPropagation(); setModalImage(m.image_url); }}
                                    style={{
                                      width: 48, height: 48, objectFit: 'cover',
                                      borderRadius: 6, border: '1px solid var(--border)',
                                      display: 'block', flexShrink: 0, cursor: 'pointer',
                                    }}
                                  />
                                )}
                                <span style={{ wordBreak: 'break-word', lineHeight: 1.5 }}>
                                  {m.message || (
                                    <span style={{ color: 'var(--text-muted)', fontStyle: 'italic' }}>image only</span>
                                  )}
                                </span>
                              </div>
                            </td>
            ...
            {msgsLoading && messages.length > 0 && (
              <div style={{
                ...
                gap: 10, padding: '16px 0', color: 'var(--text-muted)', fontSize: 13,
              }}>
            ...
            {panelTab === 'new' && (
              <div style={{ padding: 14 }}>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          background: isActive ? `${a.color}18` : 'var(--surface-alt)',
                          cursor: isDisabled ? 'not-allowed' : 'pointer',
                          opacity: isDisabled && !isRunning ? 0.5 : 1,
                          transition: 'all 0.15s',
                        }}
                      >
                        <span style={{ fontSize: 13, fontWeight: 600, color: isActive ? a.color : 'var(--text)', marginBottom: 3 }}>
                          {isRunning ? (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              <span style={{
                                width: 10, height: 10, border: '2px solid var(--border)',
                                borderTopColor: a.color, borderRadius: '50%',
                ...
                <div style={{ marginTop: 10, fontSize: 12, color: 'var(--text-muted)', textAlign: 'center' }}>
                  ...
                  {selectedCount > 0 && !running && !result &&
                  ...
                  <div style={{
                    ...
                    {running && (
                      <div style={{
                        ...
                        alignItems: 'center', gap: 10, padding: '28px 0',
                      }}>
                        <div className="spinner" />
                        <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                          Running {ACTION_CONFIG.find(a => a.key === running)?.label}…
                        </span>
                      </div>
                    )}
                    {resultError && !running && (
                      <div className="error-banner" style={{ margin: 0 }}>{resultError}</div>
                    )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                          ...
                          {activeActionCfg && (
                            <span style={{
                              ...
                              {result.messages?.length ?? selectedCount} msg{(result.messages?.length ?? selectedCount) !== 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
                        <ResultComponent data={result.data} messages={result.messages ?? []} />
                      </>
                    )}
                  </div>
                )}
